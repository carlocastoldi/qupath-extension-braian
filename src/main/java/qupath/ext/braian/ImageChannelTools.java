...
public class ImageChannelTools {
    ...
    public ImageStatistics getChannelStats(int resolutionLevel) throws IOException {
        double downsample = this.getServer().getDownsampleForResolution(Math.min(this.getServer().nResolutions()-1, resolutionLevel));
        RegionRequest request = RegionRequest.createInstance(this.getServer(), downsample);
        PathImage<ImagePlus> pathImage = IJTools.convertToImagePlus(this.getServer(), request);
